...
- **Аутентификация:** регистрация с email, вход по логину или email (кастомный backend)

## Изменения в ходе реализации
- Регионы переведены с отдельных городов на области; добавлена Свердловская область.
- Добавлено поле «тип почвы» на уровне региона (типичная) и участка (фактическая) —
  движок учитывает почву участка при советах по поливу.
- Связь «участок-культуры» реализована через промежуточную модель **Planting** со статусом
  и датой посадки, что позволило разделить советы для планируемых и уже посаженных культур.
- Добавлены сроки посадки (месяцы) у культур — рекомендации учитывают сезон, а не только температуру.
- Добавлен расчёт ожидаемой даты сбора урожая (дата посадки + период вегетации).